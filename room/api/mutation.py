...
from room.api.input import CategoryCreateInput, RoomCreateInput, EntryCreateInput, EntryUpdateVotesInput, \
    MergeEntriesInput, DismergeEntriesInput, UpdateEntriesCategoryInput, DeleteEntryInput
...
class Mutation:
    ...
    async def merge_entries(self, info: Info, data: MergeEntriesInput) -> EntryType:
        ...
        votes = 0
        parent_entry = await Entry.objects.acreate(title=data.title, content=data.content, category_id=data.category_id,
                                                   room_id=room_id)
